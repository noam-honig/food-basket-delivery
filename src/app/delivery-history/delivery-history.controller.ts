    ...
    let queryText = await sql.build(
      ...
      (await sql.build(
        ...
        [
          ...
          sql.build(
            'sum (case when ',
            sql.eq(fd.courierAssignUser, fd.courier),
            ' and ',
            sql.and(fd.where({ deliverStatus: DeliveryStatus.isSuccess() })),
            ' then 1 else 0 end) selfassigned'
          ),
          sql.build(
            'sum (',
            sql.case(
          ...
          sql.build(
            sql.func(
              'to_char',
              sql.func('min', fd.deliveryStatusDate),
              "'YY-MM'"
            ),
            ' firstd'
          ),
          sql.build(
            sql.func(
              'to_char',
              sql.func('max', fd.deliveryStatusDate),
              "'YY-MM'"
            ),
            ' lastd'
          )
        ...
        ' where ',
        sql.and(r)
      )) + (await sql.build(' group by ', fd.courier)),
      ') x'
    )
