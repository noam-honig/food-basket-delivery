...
export class DeliveryHistoryComponent implements OnInit {
  ...
  constructor(
    ...
    this.helperInfo = new GridSettings(
  ...
  private async refreshHelpers() {
    var x = await DeliveryHistoryController.getHelperHistoryInfo(
    ...
    let rows: any[] =
      this.helperStorage.rows[
        await remult.repo(helperHistoryInfo).metadata.getDbName()
      ]
    x = x.map((x) => {
  ...
  deliveries = new GridSettings(remult.repo(FamilyDeliveries), {
    ...
    gridButtons: [
      ...
      },
      {
        ...
        click: async () => {
          ...
          await saveToExcel(
            this.settings,
            remult.repo(FamilyDeliveries),
            this.deliveries,
            this.settings.lang.deliveries,
            this.dialog,
    ...
    columnSettings: (d) => {
      ...
      for (const c of d) {
        if (!r.includes(c) && c != d.id && c != d.family) r.push(c)
      }
    ...
    where: () => {
      ...
      return {
        deliveryStatusDate: { '>=': this.dateRange.fromDate, '<': toDate },
        distributionCenter: this.dialog.filterDistCenter(),
        deliverStatus: this.onlyDone
