...
export class ImportFromExcelController {
      ...
      let save = async () => {
        ...
        if (createDelivery) {
          ...
          if (getSettings().isSytemForMlt) {
            fd.distributionCenter = await remult.context.findClosestDistCenter(
  ...
  @BackendMethod({ allowed: Roles.familyAdmin })
  static async updateColsOnServer(
    rowsToUpdate: excelRowInfo[],
    columnMemberName: string,
    addDelivery: boolean,
    compareBasketType: boolean
  ) {
    for (const r of rowsToUpdate) {
      await ImportFromExcelController.actualUpdateCol(
        r,
        columnMemberName,
        addDelivery,
        compareBasketType,
  ...
  static async actualUpdateCol(
    i: excelRowInfo,
    entityAndColumnName: string,
    addDelivery: boolean,
    compareBasketType: boolean,
    settings: ApplicationSettings
  ) {
    let c = ImportFromExcelController.actualGetColInfo(i, entityAndColumnName)
    if (c.existingDisplayValue == c.newDisplayValue) return
    ...
    c.existingDisplayValue = await getColumnDisplayValue(
      columnFromKey(f, fd, entityAndColumnName)
    )
  ...
  @BackendMethod({ allowed: Roles.familyAdmin })
  static async checkExcelInput(
    excelRowInfo: excelRowInfo[],
    columnsInCompareMemeberName: string[],
    compareBasketType: boolean
  ) {
    ...
    for (const info of excelRowInfo) {
      ...
      let findDuplicate = async (w: EntityFilter<Families>) => {
        if (info.duplicateFamilyInfo.length == 0)
          info.duplicateFamilyInfo = (
            await remult.repo(Families).find({
              where: {
      ...
      if (info.idInHagai) await findDuplicate({ id: info.idInHagai })
      else {
        ...
        if (info.duplicateFamilyInfo.length == 0) {
          info.duplicateFamilyInfo = await Families.checkDuplicateFamilies(
            info.name,
            info.tz,
            info.tz2,
            info.phone1ForDuplicateCheck,
            info.phone2ForDuplicateCheck,
            info.phone3ForDuplicateCheck,
            info.phone4ForDuplicateCheck,
            undefined,
            true,
            info.address
          )
          if (info.duplicateFamilyInfo.length > 1) {
            if (
              info.duplicateFamilyInfo.find((f) => f.nameDup && f.sameAddress)
            ) {
              info.duplicateFamilyInfo = info.duplicateFamilyInfo.filter(
                (f) => !onlyNameMatch(f)
              )
            }
          }
      ...
      if (
        ...
        RemovedFromListExcelImportStrategy.ignore
      ) {
        info.duplicateFamilyInfo = info.duplicateFamilyInfo.filter(
          (x) => !x.removedFromList
        )
      ...
      if (!info.duplicateFamilyInfo || info.duplicateFamilyInfo.length == 0) {
        ...
        let existingFamily
        ;({ ef: existingFamily, hasDifference } = await compareValuesWithRow(
          info,
          info.duplicateFamilyInfo[0].id,
          compareBasketType,
          columnsInCompareMemeberName
        ))
        if (existingFamily.status == FamilyStatus.RemovedFromList) {
          ...
          result.updateRows.push(info)
        } else result.identicalRows.push(info)
      }
    }
    return result
  }
}
export async function compareValuesWithRow(
  info: excelRowInfo,
  withFamily: string,
  compareBasketType: boolean,
  columnsInCompareMemeberName: string[]
  ...
  let distCenter = await remult
    .repo(DistributionCenters)
    .findId(info.distCenter)
  ...
  if (!fd) {
    ...
    if (compareBasketType) basketType = basketType
    ...
    if (upd.existingDisplayValue != upd.newDisplayValue) {
      if (col == ef.$.groups) {
        ...
        if (!newVal || newVal.toString() == '') {
          ...
          let existingArray = existingString
            .toString()
            .split(',')
            .map((x) => x.trim())
          for (const val of newVal.split(',').map((x) => x.trim())) {
            if (existingArray.indexOf(val) < 0) existingArray.push(val)
          }
