...
import {
  compareValuesWithRow,
  excelRowInfo,
  getColumnDisplayValue,
  ImportFromExcelController
} from './import-from-excel.controller'
...
export class ImportFromExcelComponent implements OnInit {
  ...
  getColInfo(i: excelRowInfo, col: columnInCompare) {
    return ImportFromExcelController.actualGetColInfo(
      i,
      keyFromColumnInCompare(col)
    )
  }
  ...
  async addAll() {
    ...
    if (
      await this.dialog.YesNoPromise(
        use.language.shouldAdd + ' ' + count + ' ' + use.language.families + '?'
      )
    ) {
      this.busy.doWhileShowingBusy(async () => {
        try {
          ...
          for (const i of rows) {
            ...
            if (rowsToInsert.length == 35) {
              if (new Date().valueOf() - lastDate > 10000) {
                let timeLeft =
                  (((new Date().valueOf() - start) / index) * (count - index)) /
                  1000 /
                  60
                this.dialog.Info(
                  i.rowInExcel +
                    ...
                    timeLeft.toFixed(1) +
                    ' ' +
                    use.language.minutesRemaining
                )
              }
              await ImportFromExcelController.insertRows(
                rowsToInsert,
                this.addDelivery
              )
          ...
          this.newRows = this.newRows.filter(
            (x) => this.identicalRows.indexOf(x) < 0
          )
        }
        ...
        if (await this.dialog.YesNoPromise(use.language.gotoDeliveriesScreen))
          this.routeHelper.navigateToComponent(
            (await import('../families/families.component')).FamiliesComponent
          )
      })
    }
  }
  ...
  async updateAllCol(col: columnInCompare) {
    ...
    let message =
      use.language.shouldUpdateColumn +
      ' ' +
      col.c.caption +
      ' ' +
      use.language.for +
      ...
      this.busy.doWhileShowingBusy(async () => {
        ...
        for (const i of this.updateRows) {
          ...
          if (rowsToUpdate.length == 35) {
            allRows.push(
              ...(await ImportFromExcelController.updateColsOnServer(
                rowsToUpdate,
                keyFromColumnInCompare(col),
                this.addDelivery,
                this.compareBasketType
              ))
            )
  ...
  async updateCol(i: excelRowInfo, col: columnInCompare) {
    let r = await ImportFromExcelController.updateColsOnServer(
      [i],
      keyFromColumnInCompare(col),
      this.addDelivery,
      this.compareBasketType
    ...
    fileReader.onload = async (e: any) => {
      try {
        await this.busy.doWhileShowingBusy(async () => {
          ...
          if (sheets.length > 1) {
            ...
            await openDialog(SelectListComponent, (x) => {
              x.args = {
                ...
                options: sheets.map(
                  (x) => ({ name: x, item: x } as selectListItem)
                ),
          ...
          if (!sRef)
            throw (
              use.language.excelSheel +
              ' "' +
              this.sheet +
              '" ' +
              use.language.excelSheetIsEmpty
            )
          ...
          if (this.totalRows > 500000) this.totalRows = 0
          let colPrefix = ''
          let colName = 'A'
          let colIndex = 0
          this.excelColumns = []
          while (true) {
            ...
            if (this.excelColumns.length > 100) break
          }
      ...
      if (prevName == this.filename) this.loadSettings()
      this.stepper.next()
    }
  ...
  async readLine(
    row: number,
    updatedFields: Map<FieldMetadata<any>, boolean>
  ): Promise<excelRowInfo> {
    ...
    let helper = new columnUpdateHelper(
      this.dialog,
      this.settings.excelImportAutoAddValues,
      fd
    )
    for (const c of this.excelColumns) {
      if (c.field) {
        let val = this.getTheData(c.excelField + row)
        if (val && val.length > 0) await c.field.updateFamily(val, f, helper)
      }
    }
    ...
    if (
      this.useFamilyMembersAsNumOfBaskets &&
      !updatedFields.get(this.fd.quantity)
    ) {
  ...
  async ngOnInit() {
    ...
    if (false) {
      try {
        ...
        this.columnsInCompare = JSON.parse(
          sessionStorage.getItem('columnsInCompare')
        ).map((x) => this.f[x])
    ...
    this.columns.push({
      ...
      updateFamily: async (v, f, h) => {
        h.laterSteps.push({ step: 2, what: async () => updateCol(f.$.name, v) })
      },
      columns: [this.f.name],
      searchNames: [use.language.firstNameShort]
    })
    addColumns([this.f.postalCode])
    this.columns.push({
      key: 'id',
    ...
    this.columns.push({
      ...
      updateFamily: async (v, f) => {
        if (this.settings.lang.languageCode == 'iw') {
          ...
          if (r.address) updateCol(f.$.address, r.address)
          if (r.dira) updateCol(f.$.appartment, r.dira)
          if (r.floor) updateCol(f.$.floor, r.floor)
          if (r.knisa) updateCol(f.$.entrance, r.knisa)
        } else f.address = v
      },
      columns: [
        this.f.address,
      ...
      this.columns.push({
        ...
        updateFamily: async (v, f) => {
          f.$.find(c).value = new Phone(
            fixPhone(v, this.settings.defaultPrefixForExcelImport)
          )
        }
      })
    }
    ...
    for (const c of [
      ...
      if (getCustomColumnVisible(c)) addColumn(c)
    }
    addColumns([
      this.f.iDinExcel,
      ...
      updateFamily: async (v, f, h) => {
        if (v && v.trim().length > 0) {
          let g = await remult
            ...
            .findFirst(
              { name: v.trim() },
              { createIfNotFound: true, useCache: true }
            )
          if (g.isNew()) await g.save()
        }
    ...
    this.settingsArea = new DataAreaSettings({
      fields: () => {
        ...
        result = [
          this.$.addDelivery,
          {
            field: s.$.defaultDeliveryStatusIsEnquireDetails,
            visible: () => this.addDelivery && s.usingCallModule
          },
          {
            field: this.$.defaultBasketType,
            visible: () =>
              this.addDelivery &&
              !updateColumns.get(this.fd.basketType) &&
              !updateColumns.get(this.f.basketType)
          },
          {
            field: s.$.excelImportUpdateFamilyDefaultsBasedOnCurrentDelivery,
            visible: () => this.addDelivery
          },
          { field: this.$.compareBasketType, visible: () => this.addDelivery },
          {
            ...
            visible: () =>
              this.addDelivery &&
              !updateColumns.get(this.fd.distributionCenter) &&
              this.dialog.hasManyCenters
          },
          {
            field: this.$.useFamilyMembersAsNumOfBaskets,
            visible: () =>
              this.addDelivery &&
              !updateColumns.get(this.fd.quantity) &&
              updateColumns.get(this.f.familyMembers)
          },
  ...
  async iterateExcelFile(actualImport = false) {
    ...
    await this.busy.doWhileShowingBusy(async () => {
        ...
        for (let c of e.fields) {
          if (updatedColumns.get(c)) {
            ...
            if (
              ...
              this.columnsInCompareMemberName.push(
                keyFromColumnInCompare({ e, c })
              )
            }
          }
        }
      ...
      for (const c of laterColumnsInCompare) {
        ...
        this.columnsInCompareMemberName.push(
          keyFromColumnInCompare({ e: this.fDefs, c })
        )
      }
      ...
      try {
        for (index = 2; index <= this.totalRows; index++) {
          ...
          let identical = true
          if (!previousRow) identical = false
          else
          ...
          if (identical) {
            ...
            previousRow.values[key].newDisplayValue +=
              ', ' + f.values[key].newDisplayValue
            if (rows.length == 0) {
            ...
            let exists = (
              ...
              if (x && x.rowInExcel < index) {
                f.error =
                  ...
                  origVal +
                  ' - ' +
                  use.language.alreadyExistsInLine +
                  ' ' +
                  x.rowInExcel
                f.otherExcelRow = x
                return true
            ...
            if (
              this.settings.checkIfFamilyExistsInFile &&
              (exists(f.tz, usedTz, use.language.socialSecurityNumber) ||
                (this.settings.checkDuplicatePhones &&
                  (exists(
                    f.phone1ForDuplicateCheck,
                    usedPhone,
                    use.language.phone1
          ...
          if (rows.length == 50) {
            this.dialog.Info(
              index -
                1 +
                ' ' +
                (f.name ? f.name : use.language.unnamed) +
                ' ' +
                (f.error ? f.error : '')
            )
        ...
        for (const iterator of collected) {
          if (iterator.length > 1) {
            for (const row of iterator) {
              row.error =
                use.language.sameLineExcelMatchesSeveralRowsInTheDatabase +
                ' ' +
        ...
        if (
          ...
          this.settings.lang.languageCode == 'iw'
        ) {
          let suspectAddress = 0
          for (const r of this.newRows) {
            if (!r.address || r.address.match(/[0-9]$/)) suspectAddress++
          }
          ...
          if (precent > 30 && updatedColumns.get(this.f.address)) {
            if (
              await this.dialog.YesNoPromise(
                precent.toFixed() +
                  '% ' +
                  use.language.manyAddressesEndWithNumber
              )
            ) {
        ...
        sessionStorage.setItem(
          'columnsInCompare',
          JSON.stringify(this.columnsInCompareMemberName)
        )
      } catch (err) {
        this.stepper.previous()
  ...
  private async processExcelRowsAndCheckOnServer(rows: excelRowInfo[]) {
    if (this.settings.checkIfFamilyExistsInDb) {
      let r = await ImportFromExcelController.checkExcelInput(
        rows,
        this.columnsInCompareMemberName,
        this.compareBasketType
      )
  ...
  private buildUpdatedColumns() {
    ...
    //updatedColumns.set(this.f.status, true);
    for (const cu of [
      ...this.excelColumns.map((f) => f.field),
      ...this.additionalColumns.map((f) => f.field)
    ]) {
      if (cu)
  ...
  async moveFromErrorToAdd(r: excelRowInfo) {
    ...
    if (
      await this.ask(
        use.language.moveTheFamily +
          ' ' +
          name +
          ' ' +
          use.language.toNewFamilies +
          '?'
      )
    ) {
      if (!r.name) {
        r.name = name
        r.values[keyFromColumnInCompare({ e: this.fDefs, c: this.f.name })] = {
          newValue: r.name,
          newDisplayValue: r.name
        }
      }
  ...
  async moveFromErrorToUpdate(i: excelRowInfo, f: duplicateFamilyInfo) {
    if (
      await this.ask(
        use.language.shouldCompare +
          ` "${i.name}" ` +
          use.language.compareTo +
          `" ${f.name}" ` +
          use.language.andMoveToUpdateFamilies
      )
    ) {
  ...
  async ask(what: string) {
    if (this.stopAskingQuestions) return true
    return await this.dialog.YesNoPromise(what)
  }
  private async actualMoveFromErrorToUpdate(
    i: excelRowInfo,
    f: duplicateFamilyInfo
  ) {
    let r = await compareValuesWithRow(
      i,
      f.id,
      this.compareBasketType,
      this.columnsInCompareMemberName
  ...
  async testImport() {
    if (remult.isAllowed(Roles.admin)) await this.settings.save()
    ...
    let addRows = (
      ...
      for (const f of from) {
        ...
        for (const col of this.columnsInCompare) {
          ...
          if (v)
            r[col.c.caption] = updateRows
              ? v.existingDisplayValue
              : v.newDisplayValue
          if (r[col.c.caption] === undefined) r[col.c.caption] = ''
        }
        rows.push(r)
      }
    }
...
class columnUpdateHelper {
  constructor(
    ...
    public fd: ActiveFamilyDeliveries
  ) {}
  ...
  async lookupAndInsert<T extends EntityBase, dataType, Y>(
    ...
    if (x.isNew()) {
      let s =
        ...
        val +
        '" ' +
        use.language.doesNotExist
      if (
        this.autoAdd ||
        (await this.dialog.YesNoPromise(
          s + ', ' + use.language.questionAddToApplication + '?'
        ))
      ) {
        if (additionalUpdates) additionalUpdates(x)
        await x.save()
...
export function fixPhone(phone: string, defaultPrefix: string) {
  ...
  if (
    (phone.startsWith('o') || phone.startsWith('O')) &&
    phone[1] >= '0' &&
    phone[1] <= '9'
  )
    return '0' + phone.substring(1, 100)
  if (phone.length == 8 || phone.length == 9) return '0' + phone
...
export function processPhone(input: string): phoneResult[] {
      ...
      case '|':
        if (temp.length == 0 || temp[i - 1] != char)
          if (currentText.length > 0) {
            ...
            if (temp.length > 0) temp.push(temp.pop().trim() + char)
            else temp.push(char)
          }
        break
      default:
        let d = isDigit(char)
        if (d && (currentText == 'o' || currentText == 'O')) currentText = '0'
        let only = onlyDigits(currentText)
        if (d == only || currentText == '') currentText += char
        else {
          temp.push(currentText)
          currentText = char
        }
  ...
  if (currentText > '') temp.push(currentText)
  let temp2 = []
  while (temp.length > 0) {
    let c = temp.pop()
    if (c == ' ' || c == ',' || c == '-' || c == '/') {
  ...
  for (const item of temp2) {
    ...
    if (onlyDigits(item)) {
      ...
      } else current.comment = item
    }
  }
  if (current.phone) result.push(current)
...
export function parseAndUpdatePhone(
  input: string,
  f: Families,
...
function onlyDigits(input: string) {
  for (let index = 0; index < input.length; index++) {
    ...
    if (element != ' ' && !isDigit(element)) return false
  }
  return true
}
