...
import {
  ApplicationSettings,
  RemovedFromListExcelImportStrategy,
  setSettingsForSite
...
export async function initSchema(dataSource: SqlDatabase, org: string) {
  ...
  if (!settings.reminderSmsText) settings.reminderSmsText = l.reminderSmsText
  ...
  if (!settings.boxes1Name) settings.boxes1Name = l.boxes1Name
  if (!settings.boxes2Name) settings.boxes2Name = l.boxes2Name
  await settings.save()
  ...
  if (settings.dataStructureVersion == 10) {
    ...
    settings.removedFromListStrategy =
      RemovedFromListExcelImportStrategy.displayAsError
  ...
  if (settings.dataStructureVersion == 11) {
    await dataSource.execute(
      await sql.build(
        'create index if not exists fd_1 on ',
        fd,
        ' (',
        [fd.family, fd.deliveryStatusDate, fd.deliverStatus, fd.courier],
        ')'
      )
    )
    ...
    await dataSource.execute(
      await sql.build('drop index if exists for_courier  ')
    )
    await dataSource.execute(
      await sql.build('drop index if exists for_distribution_status_queries  ')
    )
    await dataSource.execute(await sql.build('drop index if exists for_name  '))
    await dataSource.execute(
      await sql.build('drop index if exists for_courier1  ')
    )
    await dataSource.execute(
      await sql.build('drop index if exists for_distribution_status_queries1  ')
    )
    await dataSource.execute(
      await sql.build('drop index if exists for_basket  ')
    )
    await dataSource.execute(
      await sql.build('drop index if exists for_basket_dist  ')
    )
    await createDeliveryIndex(
      'for_courier2',
      fd.courier,
    ...
    await createDeliveryIndex(
      'for_distribution_status_queries2',
            ...
            let r: [FieldMetadata, any][] = [
              ...
              [
                ...
                sql.case(
                  [
                    {
                      when: ['deliverStatus in (0,2)'],
                      then: 'deliveryStatusDate'
                    }
                  ],
                  'courierAssingTime'
                )
  ...
  await version(17, async () => {
    await dataSource.execute(
      await sql.build('drop index if exists for_distCenter_name  ')
    )
  })
  await version(18, async () => {
    await dataSource.execute(
      await sql.update(f, { set: () => [[f.quantity, 1]] })
    )
  })
  await version(19, async () => {
    await dataSource.execute(
      await sql.update(fd, { set: () => [[fd.quantity, 1]] })
    )
  })
  await version(20, async () => {
    let dc = await remult
      .repo(DistributionCenters)
      .find({ where: { name: 'נקודת חלוקה ראשונה' } })
  ...
  await version(21, async () => {
    if ((await remult.repo(Families).count()) > 0)
      await dataSource.execute(
        await sql.update(fd, {
          set: () => [
            [fd.fixedCourier, f.fixedCourier],
            [fd.familyMembers, f.familyMembers]
          ],
          from: f,
  ...
  await version(22, async () => {
    await pagedRowsIterator(remult.repo(Families), {
      ...
      forEachRow: async (f) => {
        ...
        if (f.addressOk) await f.save()
      }
    })
  })
  await version(23, async () => {
    let r = await dataSource.execute(
      await sql.query({
        from: fd,
        select: () => [fd.id],
        innerJoin: () => [{ to: f, on: () => [sql.eq(f.id, fd.family)] }],
        where: () => [
          fd.where(FamilyDeliveries.active),
          sql.or(
            sql.ne(fd.addressByGoogle, f.addressByGoogle),
            sql.ne(fd.addressOk, f.addressOk)
          )
        ]
      })
    )
  ...
  await version(24, async () => {
    settings.forWho = settings._old_for_soliders
      ? TranslationOptions.soldiers
      : TranslationOptions.Families
    await settings.save()
  })
  ...
  await version(26, async () => {
    settings.successMessageText =
      'שלום !משפחה!, אחד המתנדבים שלנו מסר לכם סל. בברכה !ארגון!'
    await settings.save()
  })
  ...
  await version(36, async () => {
    settings.questionForVolunteerWhenUploadingPhoto =
      settings.lang.defaultQuestionForVolunteerWhenUploadingPhoto
    await settings.save()
  })
  ...
  await version(39, async () => {
    ...
    if (
      settings.lang.defaultQuestionForVolunteerWhenUploadingPhoto !=
      l.defaultQuestionForVolunteerWhenUploadingPhoto
    ) {
      settings.questionForVolunteerWhenUploadingPhoto =
        settings.lang.defaultQuestionForVolunteerWhenUploadingPhoto
      await settings.save()
  ...
  await version(41, async () => {
    ...
    if ((await repo.count()) > 0)
      await dataSource.execute(
        await sql.update(com, {
          set: () => [[com.message, 'comment']]
        })
      )
  })
...
להסרה מהרשימה השב הסר`
  ...
  await version(44, async () => {
    const message = await remult
      ...
      .findId('attendanceReminder', { createIfNotFound: true })
    message.template =
      'הי !מתנדב!, מזכירים את ההתנדבות שתואמה להיום, בהוקרה !ארגון!'
  })
  ...
  await version(48, async () => {
    ...
    await dataSource.execute(
      await sql.build(
        'create index if not exists ComHistory_by_volunteer on ',
        h,
        '  (',
        [h.volunteer, h.createDate],
        ')'
      )
    )
  })
