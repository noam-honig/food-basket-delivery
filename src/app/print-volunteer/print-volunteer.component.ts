...
import {
  ...
  Property,
  SizeProperty
} from '../print-stickers/VolunteerReportDefs'
...
export class PrintVolunteerComponent implements OnInit {
  ...
  pageBreakBefore() {
    if (this.report.page[this.newPageKey]) return 'always'
    return 'none'
  }
  ...
  columnProps: ElementProps = {
    caption: remult.context.lang.columnProperties,
    props: [
  ...
  addColumn() {
    this.report.columns.push({
      controls: [],
      propertyValues: {
        [this.defs.textBeforeKey]: remult.context.lang.newColumn
      }
    })
  ...
  editColumn(c: ReportColumn) {
    ...
    this.currentProps.caption =
      remult.context.lang.columnProperties +
      ': ' +
      c.propertyValues[this.defs.textBeforeKey]
  ...
  moveControl(pos = 1) {
    ...
    let to = from + pos
    if (to >= this.currentControlList.length || to < 0) return
    this.currentControlList.splice(
      to,
      0,
      ...this.currentControlList.splice(from, 1)
    )
    this.save()
  }
  removeColumn() {
    this.report.columns.splice(
      this.report.columns.indexOf(this.currentColumn),
      1
    )
    this.save()
  }
  moveColumn(pos = 1) {
    ...
    let to = from + pos
    if (to >= this.report.columns.length || to < 0) return
  ...
  async ngOnInit() {
    ...
    this.data = [
      ...this.data.filter((x) => x.firstRow[this.defs.helperPhoneKey]),
      ...this.data.filter((x) => !x.firstRow[this.defs.helperPhoneKey])
    ]
    for (const item of this.data) {
      if (!item.firstRow[this.defs.helperPhoneKey])
        item.deliveries.sort((a, b) =>
          a[this.defs.nameKey].toString().localeCompare(b[this.defs.nameKey])
        )
      let totalItems = new quantityHelper()
      let totalBaskets = new quantityHelper()
      item.deliveries.forEach((d) => {
        totalBaskets.parseComment(
          d[this.defs.basketTypeKey],
          +d[this.defs.quantityKey]
        )
