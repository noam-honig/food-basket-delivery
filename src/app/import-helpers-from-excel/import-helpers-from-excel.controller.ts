...
export class ImportHelpersFromExcelController {
  ...
  @BackendMethod({ allowed: Roles.admin })
  static async updateHelperColsOnServer(
    rowsToUpdate: excelRowInfo[],
    columnMemberName: string
  ) {
    for (const r of rowsToUpdate) {
      await ImportHelpersFromExcelController.actualUpdateCol(
        r,
        columnMemberName
      )
    }
  ...
  @BackendMethod({ allowed: Roles.admin })
  static async checkExcelInputHelpers(
    ...
    for (const info of excelRowInfo) {
      info.duplicateHelperInfo = (
        await remult
          .repo(Helpers)
          .find({ where: { phone: new Phone(info.phone) } })
      ).map((x) => {
        return {
          id: x.id,
          name: x.name
      ...
      if (!info.duplicateHelperInfo || info.duplicateHelperInfo.length == 0) {
        ...
        let ef = await remult
          .repo(Helpers)
          .findId(info.duplicateHelperInfo[0].id)
