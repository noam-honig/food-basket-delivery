...
import {
  ...
  phoneResult,
  parseAndUpdatePhone
} from './import-from-excel/import-from-excel.component'
import {
...
describe('AppComponent', () => {
      ...
      await sql.query({
        ...
        where: async () => [
          ...
          sql.build(
            f.id,
            ' in (',
            await sql.query({ select: () => [afd.family], from: afd }),
            ')'
          )
        ]
      })
    ).toBe(
      'select e1.id from Families e1 where status <> 98 and e1.id in (select fd.family from FamilyDeliveries fd where archive = false)'
    )
  })
  it('test custom filter', async () => {
    expect(await sql.build(fd.where(FamilyDeliveries.onTheWayFilter()))).toBe(
      "deliverStatus = 0 and courier <> ''"
    )
  })
  it('test build', async () => {
  ...
  it('fixed filter 2', async () => {
    expect(
      await sql.columnInnerSelect(afd, {
      ...
      '(select FamilyDeliveries.deliverStatus s from FamilyDeliveries FamilyDeliveries where FamilyDeliveries.family = 123 and archive = false order by FamilyDeliveries.deliveryStatusDate desc limit 1)'
    )
  })
  it('fixed filter 3', async () => {
    expect(
      await sql.columnInnerSelect(fd, {
      ...
      '(select FamilyDeliveries.deliverStatus s from FamilyDeliveries FamilyDeliveries where FamilyDeliveries.family = 123 order by FamilyDeliveries.deliveryStatusDate desc limit 1)'
    )
  })
  it('Where', () => {
    q(
  ...
  it('another where', async () => {
    let b = remult.repo(BasketType).create({ id: '11', name: 'basket' })
    expect(await sql.build(fd.where({ basketType: b }))).toBe(
      "basketType = '11'"
    )
  })
  it('group By', () => {
        ...
        innerJoin: async () => [
          {
            to: f,
            on: async () => [
              sql.build(await f.basketType.getDbName(), ' = ', bt.id)
            ]
          }
        ]
      },
  ...
  it('column dbname can reference root entity', async () => {
    ...
    expect(
      await sql.columnSumInnerSelect(bt, f.familyMembers, {
        from: f,
        where: () => [sql.eq(f.basketType, bt.id)]
      })
    ).toBe(
      '(select sum(e1.familyMembers) from Families e1 where e1.basketType = BasketType.id)'
    )
  })
  it('case ', async () => {
    expect(
    ...
    ).toBe('case when 1=1 and 2=2 then 3 when 3=3 then 4 else 9 end')
  })
  it('delete 2', async () => {
    let p = SqlFor(remult.repo(BasketType))
    expect(await sql.delete(p, sql.eq(p.boxes, 5), sql.eq(p.boxes, 6))).toBe(
      'delete from BasketType where boxes = 5 and boxes = 6'
    )
  })
  it('update ', async () => {
    expect(
      ...
      "update BasketType p set id = '123', name = 'noam' where p.boxes = 5 and p.boxes = 6"
    )
  })
  it('update 2 ', async () => {
    ...
    expect(
      ...
      "update BasketType p set id = e2.basketType, name = 'noam' from Families e2 where p.boxes = 5 and p.boxes = e2.familyMembers"
    )
  })
  it('insert ', async () => {
    sql = new SqlBuilder()
    ...
    expect(
      ...
      "insert into BasketType (id, name) select e1.id, 'noam' from Families e1 where e1.familyMembers = 5"
    )
  })
  it('filter ', async () => {
    expect(await sql.build(bt.where({ boxes: 3, $and: [{ boxes: 5 }] }))).toBe(
      'boxes = 3 and boxes = 5'
    )
  })
  it('parse address', () => {
  ...
  it('test address parser', () => {
    expect(
      parseUrlInAddress(
        'https://maps.google.com/maps?q=32.53666305541992%2C34.905311584472656&z=17&hl=en'
      )
    ).toBe('32.53666305541992,34.905311584472656')
    expect(
      parseUrlInAddress(
        "https://www.google.com/maps/place/32%C2%B032'12.0%22N+34%C2%B054'19.1%22E/@32.5366631,34.9031229,17z/data=!3m1!4b1!4m5!3m4!1s0x0:0x0!8m2!3d32.5366631!4d34.9053116?hl=en"
      )
    ).toBe('32.5366631,34.9053116')
    expect(
      parseUrlInAddress(
        "https://www.google.com/maps/place/32%C2%B032'01.9%22N+34%C2%B054'14.3%22E/@32.5338692,34.9055186,17z/data=!3m1!4b1!4m14!1m7!3m6!1s0x0:0x0!2zMzLCsDMyJzEyLjYiTiAzNMKwNTQnMjAuMiJF!3b1!8m2!3d32.5368337!4d34.9056189!3m5!1s0x0:0x0!7e2!8m2!3d32.533866!4d34.9039709?hl=iw"
      )
    ).toBe('32.533866,34.9039709')

    expect(
      parseUrlInAddress(
        `מיקום:32.267417990635636,34.878578873069586דיוק: 20.679268441942863`
      )
    ).toBe('32.267417990635636,34.878578873069586')
    expect(parseUrlInAddress('32.267417990635636,34.878578873069586')).toBe(
      '32.267418,34.878579'
    )
    expect(parseUrlInAddress('נועם')).toBe('נועם')
  })
  it('format phone', () => {
    expect(TranslationOptions.Families.formatPhone('214,391,757')).toBe(
      '021-439-1757'
    )
  })
  it('fix phone input', () => {
  ...
  testPhone('0532777561 // 0532777561', [
    { phone: '0532777561', comment: '' },
    { phone: '0532777561', comment: '' }
  ])
  testPhone('04-8767772 / 050-7467774 (לריסה)', [
    { phone: '04-8767772', comment: '' },
    { phone: '050-7467774', comment: '(לריסה)' }
  ])
  ...
  testPhone('0533377277- אלי אחיו של יעקב', [
    { phone: '0533377277', comment: 'אלי אחיו של יעקב' }
  ])
  testPhone('+772 57-777-7572', [{ phone: '+77257-777-7572', comment: '' }])
  testPhone('0537777237 או 057-7227322(של שותף)', [
    { phone: '0537777237', comment: 'או' },
    { phone: '057-7227322', comment: '(של שותף)' }
  ])
  testPhone('טלפון איש קשר 0523773707 אורלי', [
    { phone: '0523773707', comment: 'טלפון איש קשר אורלי' }
  ])
  testPhone('חרשים -הבן יוסף 057-5773777-    03-7377772', [
    { comment: 'חרשים-הבן יוסף', phone: '057-5773777' },
    { phone: '03-7377772', comment: '' }
  ])
  testPhone('אמא רויטל 057-2225337', [
    { phone: '057-2225337', comment: 'אמא רויטל' }
  ])
  testPhone('057 - 5005570', [{ phone: '057-5005570', comment: '' }])
  testPhone('טלפון של סימי- אחות: 0527323705', [
    { phone: '0527323705', comment: 'טלפון של סימי-אחות:' }
  ])
  testPhone('נהוראי-אבא: 052-7777737', [
    { phone: '052-7777737', comment: 'נהוראי-אבא:' }
  ])
  testPhone('7337777 - 057', [{ phone: '057-7337777', comment: '' }])
  testPhone('מלכי ישראל 770/77', [{ phone: 'מלכי ישראל 770/77', comment: '' }])
  testPhone('0507330590/1', [
    { phone: '0507330590', comment: '' },
    { phone: '0507330591', comment: '' }
  ])
  testPhone('0507330590/81', [
    { phone: '0507330590', comment: '' },
    { phone: '0507330581', comment: '' }
  ])
  testPhone('052-737-7703 052-737-7703', [
    { phone: '052-737-7703', comment: '' },
    { phone: '052-737-7703', comment: '' }
  ])
  testPhone('7322575 – 057', [{ phone: '057-7322575', comment: '' }])
  testPhone('050-7330590 | 050-7953019', [
    { phone: '050-7330590', comment: '' },
    { phone: '050-7953019', comment: '' }
  ])
  testPhone('0532777561 // 0532777561', [
    { phone: '0532777561', comment: '' },
    { phone: '0532777561', comment: '' }
  ...
  it('properMerge4', () => {
    ...
    let c = new MergeFamiliesComponent(
      undefined,
      undefined,
      undefined,
      undefined
  ...
  it('valid phoneb', () => {
    let test = (phone: string, expected: boolean) =>
      expect(isPhoneValidForIsrael(phone)).toBe(expected, phone)
    test('039197373', true)
  })
  it('test event date', () => {
    ...
    expect(
      eventDisplayDate({ ...x, eventDateJson: '2021-08-02' }, true, today)
    ).toBe('עבר')
    expect(
      eventDisplayDate({ ...x, eventDateJson: '2021-08-03' }, true, today)
    ).toBe('היום (03/08)')
    expect(
      eventDisplayDate({ ...x, eventDateJson: '2021-08-04' }, true, today)
    ).toBe('מחר (04/08)')
    expect(
      eventDisplayDate({ ...x, eventDateJson: '2021-08-05' }, true, today)
    ).toBe('השבוע')
    expect(
      eventDisplayDate({ ...x, eventDateJson: '2021-08-10' }, true, today)
    ).toBe('שבוע הבא')
    expect(
      eventDisplayDate({ ...x, eventDateJson: '2021-08-15' }, true, today)
    ).toBe('אוגוסט')
    expect(
      eventDisplayDate({ ...x, eventDateJson: '2021-09-13' }, true, today)
    ).toBe('ספטמבר')
  })
})
