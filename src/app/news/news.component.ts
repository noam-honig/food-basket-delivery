...
export class NewsComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    public filters: NewsFilterService,
  ...
  async showHelper(n: ActiveFamilyDeliveries) {
    openDialog(
      ...
      async (s) => (s.argsHelper = n.courier)
    )
  }
  ...
  familySources: familySource[] = [
    { id: undefined, name: this.settings.lang.allFamilySources }
  ]

  async ngOnInit() {
    if (
      this.activatedRoute.routeConfig.path == NewsComponent.needsWorkRoute.path
    ) {
      this.filters.setToNeedsWork()
    }
    this.refresh()
    this.familySources.push(
      ...(
        await remult.repo(FamilySources).find({ orderBy: { name: 'asc' } })
      ).map((x) => {
        return { id: x.id, name: x.name } as familySource
      })
    )
  }
  newsRows = 50
