...
export class Event extends IdEntity {
      ...
      if (v.helper)
        if (!v.helper.doNotSendSms) {
          await new SendSmsUtils().sendSms(
            v.helper.phone.thePhone,
            this.createMessage(v).merge(
              settings.confirmEventParticipationMessage
            ),
            v.helper,
            {
              eventId: this.id
  ...
  @Field({
    ...
    sqlExpression: async (selfDefs) => {
      ...
      return sql.columnCount(self, {
        from: vie,
        where: () => [
          sql.eq(vie.eventId, self.id),
          vie.where({ canceled: false })
        ]
      })
    }
  })
    ...
    sqlExpression: async (selfDefs) => {
      ...
      return sql.columnCountWithAs(
        self,
        {
          ...
          where: () => [
            sql.eq(vie.eventId, self.id),
            vie.where({ canceled: false, confirmed: true })
          ]
        },
        'confirmed'
      )
  ...
  static async duplicateEvent(
    ui: UITools,
    events: Event[],
    done: (createdEvents: Event[]) => void
  ) {
    let settings = await remult.context.getSettings()
    let archiveCurrentEvent = new InputField<boolean>({
      valueType: Boolean,
      caption: settings.lang.archiveCurrentEvent
    })
    archiveCurrentEvent.value = true
    let date = new InputField<Date>({
      caption: settings.lang.eventDate,
  ...
  get thePhoneDisplay() {
    if (this.getPhone()?.thePhone) return this.getPhone().displayValue
  }
  get theAddress() {
    if (this.getAddress().ok) return this.getAddress().getAddress
  }
  getAddress() {
    if (this.addressHelper.ok) return this.addressHelper
    if (this.distributionCenter?.addressHelper.ok)
      return this.distributionCenter.addressHelper
    return getSettings().addressHelper
  }
  get city() {
    if (this.getAddress().ok) return this.getAddress().getCity
  }
  get longLat() {
    if (this.getAddress().ok) return this.getAddress().getlonglat
  }
}
...
@Entity<volunteersInEvent>('volunteersInEvent', {
  ...
  apiPrefilter: () => ({
    helper: !remult.isAllowed([Roles.admin, Roles.distCenterAdmin])
      ? { $id: [remult.user?.id] }
      : undefined
  }),
  saving: async (self) => {
...
export class volunteersInEvent extends IdEntity {
  ...
  @Field({
    ...
    sqlExpression: async (selfDefs) => {
      ...
      return sql.columnCount(self, {
        from: d,
        where: () => [sql.eq(self.helper, d.courier)]
      })
    }
  })
    ...
    sqlExpression: async (selfDefs) => {
      ...
      return sql.columnCountWithAs(
        self,
        {
          from: d,
          where: () => [
            sql.eq(self.helper, d.courier),
            d.where({ deliverStatus: DeliveryStatus.isSuccess() })
          ]
        },
        'succesfulDeliveries'
      )
  ...
  @Field({
    sqlExpression: async (selfDefs) => {
      ...
      let self = SqlFor(selfDefs)
      let d = SqlFor(remult.repo(FamilyDeliveries))
      return sql.columnMaxWithAs(
        self,
        d.courierAssingTime,
        { from: d, where: () => [sql.eq(self.helper, d.courier)] },
        'lastAssignTime'
      )
    }
  })
      ...
      remult.repo(volunteersInEvent),
      {
        ...
        columnSettings: (ev: FieldsMetadata<volunteersInEvent>) => [
          { width: '100', field: ev.helperName, readonly: true },
          {
            ...
            getValue: (v) => {
              if (v.canceled) return use.language.canceledParticipation
              let additionalText = ''
              if (v.confirmed)
              ...
              if (v.assignedDeliveries > 0)
                ...
                else return getLang().assigned + additionalText
        ...
        rowCssClass: (v) => {
          ...
          if (v.assignedDeliveries > 0)
            if (v.lastAssignTime < v.lastSmsTime) return 'deliveredOk'
            else return 'largeDelivery'
          else if (v.succesfulDeliveries == 0) return 'newVolunteer'
          return ''
        },
        gridButtons: [
          {
            ...
            click: async () => {
              ui.editCustomMessageDialog({
                helpText:
                  'פעולה זו תשלח הודעה לכל מתנדבי הארוע ותבקש מהם לאשר הגעה.  אם המתנדב ישיב "כן", ירשם שהוא אישור הגעה, אם השיב "לא" ירשם שביטל השתתפות ואם השיב "הסר" יוסר מרשימות התפוצה',
                    ...
                    click: async ({ templateText, close }) => {
                      ...
                      if (
                        await ui.YesNoPromise(
                          'לשלוח הודעה לכל המתנדבים שנרשמו?'
                        )
                      ) {
                        settings.confirmEventParticipationMessage = templateText
                        await settings.save()
                        let result =
                          await event.sendParticipationConfirmMessage()
                        ui.Info(result)
                        close()
                      }
                    }
                  ...
                  },
                  {
                    ...
                    click: async ({ templateText, close }) => {
                      ...
                      if (
                        ...
                        await message.save()
                        let result =
                          await event.sendParticipationReminderMessageMessage()
                        ui.Info(result)
                        close()
                      }
                    }
          ...
          },
          {
            ...
            click: async () => {
              saveToExcel(
                ...
                remult.repo(volunteersInEvent),
                gridSettings,
                use.language.volunteersRegisteredTo + ' ' + event.name,
                ui,
                (e, c) =>
                  c == e.$.id ||
                  c == e.$.eventId ||
        ...
        rowButtons: [
          ...
          {
            textInMenu: (x) =>
              x.duplicateToNextEvent
                ? getLang().unmarkAsFixed
                : getLang().markAsFixed,
            icon: 'person',
            click: async (ev) => {
...
export function eventDisplayDate(
  ...
  if (e.eventDateJson) {
    ...
    today = ValueConverters.DateOnly.fromJson(
      ValueConverters.DateOnly.toJson(today)
    )
    ...
    if (d > t - day) {
      ...
      if (group) {
        ...
        if (d < endOfWeek + day * 7) return use.language.nextWeek
