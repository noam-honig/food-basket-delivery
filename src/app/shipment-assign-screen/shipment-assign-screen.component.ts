...
} from './shipment-assign-screen.controller'
...
export class ShipmentAssignScreenComponent implements OnInit {
  ...
  matchesFilter(f: familyInfo) {
    if (!this.filterBasket.value || this.filterBasket.value.length == 0)
      return true
    return f.deliveries.find((b) =>
      b.basketTypeName.includes(this.filterBasket.value)
    )
  }
  ...
  private sortList() {
    this.families.sort((a, b) => {
      ...
      if (alen == 0 && blen == 0) return itemsDiff

      if (alen == 0) return 1
      if (blen == 0) return -1

      res = (alen - blen) * this.sortDir
      if (res == 0) res = itemsDiff
      ...
      (x) =>
        (x.args = {
          ...
          onSelect: async (selectedHelper) => {
            ...
            if (!h) {
              h = ShipmentAssignScreenController.helperInfoFromHelper(
                await remult.repo(Helpers).findId(selectedHelper.id)
              )
              this.data[h.id] = h
            }
      ...
      if (
        ...
        for (const key in this.data.helpers) {
          if (Object.prototype.hasOwnProperty.call(this.data.helpers, key)) {
            ...
            if (
              helper.families.length < 4 &&
              !helper.problemFamilies[family.id]
            ) {
              let d = 99999999
              let referencePoint = ''
