...
export class EventCardComponent implements OnInit {
  ...
  showEventType(e: EventInList) {
    return !this.isGeneralEvent(e) && e.type != EventType.other
  }
  isGeneralEvent(e: EventInList) {
    return isGeneralEvent(e)
  }
  getStatus(e: EventInList) {
    if (e instanceof Event)
      if (e.eventStatus != eventStatus.active) return e.eventStatus.caption
    return ''
  }
  isAdmin() {
  ...
  refresh() {
    ...
    for (const e of this._events) {
      if (!firstLongLat) firstLongLat = e.longLat
      if (e.longLat != firstLongLat) this.showLocation = true
    ...
    this.cities.splice(0, 0, {
      ...
      caption: use.language.entireRegion + ' - ' + this._events.length
    })
  ...
  clickButton(b: RowButton<Event>, e: EventInList) {
    if (e instanceof Event) b.click(e)
  }
  showVolunteers(e: EventInList) {
    if (e instanceof Event) e.showVolunteers(this.dialog)
  }
  edit(e: EventInList) {
    if (e instanceof Event) {
  ...
  adminVolunteers(e: EventInList) {
    if (
      ...
      e.registeredVolunteers != undefined
    )
      ...
      else return e.registeredVolunteers + ' ' + this.settings.lang.volunteers
  }
  distance(e: EventInList) {
    if (!this.volunteerLocation) return undefined
    return (
      GetDistanceBetween(this.volunteerLocation, e.location).toFixed(1) +
      ' ' +
  ...
  sortEvents() {
    if (!this.volunteerLocation)
      ...
      this.dates.forEach((d) =>
        d.events.sort(
          (a, b) =>
            GetDistanceBetween(this.volunteerLocation, a.location) -
            GetDistanceBetween(this.volunteerLocation, b.location)
        )
      )
  }
}
