...
import {
  DialogService,
  ShowDialogOnErrorErrorHandler
} from './select-popup/dialog'
...
} from './overview/transition-group'
...
@NgModule({
  ...
  providers: [
    ...
    },
    {
      ...
      useFactory: (service: SettingsService) => {
    ...
    try {
      try {
        injectedRemult.context.getSite = () =>
          getSiteFromUrl(window.location.pathname)
      ...
      routeMap.set(
        ImportHelpersFromExcelComponent,
        l.ImportHelpersFromExcelComponent
      )
      ...
      } else console.error('failed to get settings ', err)
    }
    return ''
  }
}
