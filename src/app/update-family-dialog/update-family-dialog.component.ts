...
export class UpdateFamilyDialogComponent
  implements OnInit, AfterViewChecked, AfterViewInit, OnDestroy
{
      ...
      .then(() => {
        if (!this.confirmed)
          if (!this.args.userCanUpdateButDontSave) {
            ...
            if (this.delivery) this.delivery._.undoChanges()
          }
      })
  }
  ngOnDestroy(): void {}
  ...
  ngAfterViewChecked(): void {
    if (!this.init && this.addressPanel) {
      if (this.args.focusOnAddress) this.addressPanel.open()
      this.init = true
  ...
  @ViewChild('deliveryPanel', { static: false })
  deliveryPanel: MatExpansionPanel
  ...
  async sendSmsToCourier() {
    ...
    h.sendSmsToCourier(
      this.dialog,
      'בקשר למשפחת "' +
        this.args.familyDelivery.name +
        '" מ "' +
        this.args.familyDelivery.address +
        '"\n'
    )
  }
  ...
  preview() {
    ...
    if (!fd) fd = this.args.family.createDelivery(this.dialog.distCenter)
    this.args.family.updateDelivery(fd)
    openDialog(PreviewFamilyComponent, (x) => {
      x.argsFamily = fd
    })
  }
  updateInfo() {
    ...
    const fields: DataAreaFieldsSetting<any>[] = [
      [f.$.createUser, f.$.createDate],
      [f.$.lastUpdateUser, f.$.lastUpdateDate],
      [f.$.statusUser, f.$.statusDate]
    ]
  ...
  async confirm() {
    if (
      !this.families.currentRow.defaultDistributionCenter &&
      this.onMapLocation
    )
      this.families.currentRow.defaultDistributionCenter =
        await remult.context.findClosestDistCenter(this.onMapLocation)
    if (this.delivery) {
      let d = this.delivery
      if (d.changeRequireStatsRefresh()) this.refreshDeliveryStatistics = true
    ...
    if (this.delivery) this.delivery._.reload()
    ...
    this.dialogRef.close()
    if (this.args && this.args.onSave) this.args.onSave()
        ...
        (x.args = {
          ...
          save: (s) => {
            if (!isGpsAddress(this.args.family.address))
              this.args.family.addressComment = (
                this.args.family.address +
                ' ' +
                this.args.family.addressComment
              ).trim()
  ...
  async editOnGoogle() {
    ...
    await c.edit(this.dialog, this.families.currentRow.address, () =>
      this.families.currentRow._.reload()
    )
  }
  images: ImageInfo[] = []
  async ngOnInit() {
    if (!this.args.familyDelivery) {
      if (this.args.deliveryId) {
        this.args.familyDelivery = await remult
          .repo(FamilyDeliveries)
          .findFirst({ id: this.args.deliveryId })
    ...
    if (!this.args.family) {
      ...
      if (this.args.familyId)
        this.args.family = await remult
          .repo(Families)
          .findFirst({ id: this.args.familyId })
    }
    const {
      addressLongitude,
      addressLatitude,
    ...
    console.log({
      ...
      drivingLatitude,
      drivingLongitude,
      loc: addressHelper.location
    })
    if (this.args.familyDelivery) this.delivery = this.args.familyDelivery
    if (this.args.userCanUpdateButDontSave) this.args.disableSave = true

    this.families.currentRow = this.args.family
    this.images = await (
      await remult
        .repo(FamilyImage)
        .find({ where: { familyId: this.args.family.id } })
    ).map(
      (i) =>
        ({
          image: i.image,
    ...
    this.extraFamilyInfo = new DataAreaSettings({
      fields: () => [
        ...
        families.email,
        [families.tz, families.tz2],
        families.familySource,
        families.socialWorker,
        [families.socialWorkerPhone1, families.socialWorkerPhone2],
        [families.custom1, families.custom2],
        [families.custom3, families.custom4],
        ...
        [
          families.birthDate,
          {
            ...
            getValue: () => {
              ...
              return Math.round(
                (new Date().valueOf() - f.birthDate.valueOf()) /
                  (365 * 86400000)
              )
            }
          }
    ...
    this.familiesAddress = this.families.addArea({
      fields: (families) => [
        [families.appartment, families.floor, families.entrance],
    ...
    this.deliveryDefaults = this.families.addArea({
      fields: (f) =>
        [
          ...
          f.fixedCourier,
          {
            field: f.defaultDistributionCenter,
            visible: () => this.dialog.hasManyCenters
          },
          f.special
        ].filter((x) =>
          this.settings.usingSelfPickupModule ? true : x != f.defaultSelfPickup
        )
    })
    if (this.delivery) {
      this.deliverInfo = new DataAreaSettings({
        fields: () => this.delivery.deilveryDetailsAreaSettings(this.dialog)
      })
    }
    if (!this.families.currentRow.isNew()) {
      ...
      new columnOrderAndWidthSaver(this.familyDeliveries).load(
        'familyDeliveriesInUpdateFamily'
      )
    }
  }
