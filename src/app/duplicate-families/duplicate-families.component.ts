...
} from './duplicate-families.controller'
...
export class DuplicateFamiliesComponent implements OnInit {
  ...
  area = new DataAreaSettings({
    fields: () => [
      [this.$.address, this.$.name, this.$.phone, this.$.tz, this.$.onlyActive]
    ]
  })
  constructor(
  ...
  async ngOnInit() {
    ...
    try {
      this.duplicateFamilies =
        await DuplicateFamiliesController.familiesInSameAddress({
        ...
        (x.args = {
          ...
          settings: new GridSettings(remult.repo(Families), {
            columnSettings: (f) => {
              ...
              for (const c of f) {
                if (!r.includes(c) && c != f.id) r.push(c)
              }
              return r
            },
            ...
            gridButtons: [
              ...[new UpdateStatus(), new updateGroup()].map((a) =>
                a.gridButton({
                  afterAction: async () => await x.args.settings.reloadData(),
                  ui: this.dialog,
                  userWhere: async () =>
                    (
                      await x.args.settings.getFilterWithSelectedRows()
                    ).where,
              ...
              ),
              {
                ...
                visible: () =>
                  ...
                  (x.args.settings.selectedRows.length > 1 ||
                    x.args.settings.totalRows < 10)
              },
              {
            ...
            rowsInPage: 25,
            where: {
              status: { $ne: FamilyStatus.ToDelete },
              id: d.ids.split(',')
            },
            orderBy: { name: 'asc' }
          })
  ...
  private async mergeFamilies(x: GridDialogComponent) {
    ...
    await openDialog(
      ...
      (y) => (y.families = items),
      (y) => {
        if (y.merged) x.args.settings.reloadData()
      }
    )
  }
}
