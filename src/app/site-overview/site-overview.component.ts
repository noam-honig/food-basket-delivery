...
export class SiteOverviewComponent implements OnInit {
  ...
  managers: Manager[] = []
  ngOnInit() {
    SiteOverviewController.siteInfo(
      this.args.site.site,
      this.args.site.isRemote
    ).then((x) => (this.managers = x))
  }
  openSite() {
  ...
  sendWhatsapp(m: Manager) {
    ...
    Phone.sendWhatsappToPhone(
      m.phone,
      this.createMessage(m).merge(message),
      true
    )
  }
  ...
  editMessage() {
    this.ui.editCustomMessageDialog({
      ...
      helpText:
        'ערוך את ההודעה כאן, ואז לחץ על שמור ובמסך הארגון לחץ על שם המנהל כדי לשלוח לו הודעה',
      buttons: [
        {
          name: 'שמור',
          click: (x) => {
