...
export class AssignEscortComponent implements OnInit {
  ...
  async searchPhone() {
    ...
    if (this.phone.length == 10) {
      let h = await remult
        .repo(HelpersAndStats)
        .findFirst({ phone: new Phone(this.phone) })
      if (h) {
        this.initHelper(h)
      } else {
  ...
  async assignForDriver(driver: HelpersAndStats) {
    ...
    if (
      await openDialog(
        ...
        (x) => (x.args = { question: 'האם גם לשלוח SMS ל' + this.helper.name }),
        (x) => x.yes
      )
    ) {
  ...
  async initHelper(h: HelpersBase) {
    if (!h.isNew()) {
      ...
      if (h instanceof HelpersAndStats)
        ...
        assignedFamilies = await (
          await remult.repo(HelpersAndStats).findFirst({ id: h.id })
          ...
          (x) =>
            (x.args = {
              question:
                'למתנדב ' +
                h.name +
                ' כבר מוגדרות משפחות, לא ניתן להגדיר אותו כמלווה',
    ...
    this.area = new DataAreaSettings<Helpers>({
      fields: () => {
        ...
        if (this.settings.showHelperComment) r.push(this.helper.eventComment)
