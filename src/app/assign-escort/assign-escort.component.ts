...
export class AssignEscortComponent implements OnInit {
  ...
  async searchPhone() {
    ...
    if (this.phone.length == 10) {
      let h = await this. remult.repo(HelpersAndStats).findFirst(h => h.phone.isEqualTo(new Phone(this.phone)));
      if (h) {
  ...
  async assignForDriver(driver: HelpersAndStats) {
    ...
    let h = await this. remult.repo(Helpers).findFirst(h => h.id.isEqualTo(driver.id));
  ...
  async clearEscort() {
    ...
    let h = await this. remult.repo(HelpersAndStats).findFirst(h => h.id.isEqualTo(this.helper.id));
    this.clearHelperInfo();
    this.initHelper(h);
  ...
  async initHelper(h: HelpersBase) {
    ...
    if (!h.isNew()) {
      ...
      if (h instanceof HelpersAndStats)
        ...
        assignedFamilies = await (await this. remult.repo(HelpersAndStats).findFirst(x => x.id.isEqualTo(h.id))).deliveriesInProgress
