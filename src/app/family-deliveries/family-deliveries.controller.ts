...
export class FamilyDeliveriesController {
  ...
  static async getDeliveriesByPhone(phoneNumIn: string) {
    ...
    let result: string[] = []
    let courier = await await remult
      .repo(Helpers)
      .findFirst({ phone: phoneNum })
    ...
    for (const d of (
      await getDb().execute(
        await sql1.query({
          ...
          where: () => [
            courier != undefined
              ? fd.where({ courier, $and: [FamilyDeliveries.active] })
              : sql1.or(
                  fd.where({
                    phone1: phoneNum,
                    $and: [FamilyDeliveries.active]
                  }),
                  fd.where({
                    phone2: phoneNum,
                    $and: [FamilyDeliveries.active]
          ...
          select: () => [sql1.columnWithAlias(fd.id, 'id')]
        })
      )
    ).rows) {
      result.push(d.id)
    ...
    return await (
      await remult.repo(FamilyDeliveries).find({ where: { id: result } })
    ).map((x) => x._.toApiJson())
  }
  @BackendMethod({ allowed: Roles.distCenterAdmin })
  static async getTotalItems(filter: any) {
    ...
    for await (const fd of remult.repo(ActiveFamilyDeliveries).query({
      where: await Filter.entityFilterFromJson<ActiveFamilyDeliveries>(
        remult.repo(ActiveFamilyDeliveries).metadata,
        filter
      )
    })) {
      ...
      if (!b) baskets.push((b = { name: fd.basketType?.name, quantity: 0 }))
