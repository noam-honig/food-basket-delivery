...
export class CustomReuseStrategy implements RouteReuseStrategy {
  ...
  shouldDetach(route: ActivatedRouteSnapshot): boolean {
    let x = (<any>route.component).prototype[
  ...
  shouldAttach(route: ActivatedRouteSnapshot): boolean {
    ...
    if (!!route.routeConfig) {
      ...
      if (h) {
        if (!h[recycleComponent]) return true
      }
    }
    return false
  }
  ...
  retrieve(route: ActivatedRouteSnapshot): DetachedRouteHandle {
    let result = null
    if (!route.routeConfig) result = null
    else {
  ...
  getRouteInfo(route: ActivatedRouteSnapshot) {
    ...
    if (route.routeConfig == null) return 'no route config'
  ...
  recycleAll() {
    for (const key in this.handlers) {
      if (this.handlers.hasOwnProperty(key)) {
        ...
        if (element) element[recycleComponent] = true
      }
    }
  }
}
export const reuseComponentOnNavigationAndCallMeWhenNavigatingToIt = Symbol(
  'reuseComponentOnNavigationAndCallMeWhenNavigatingToIt'
)
