  ...
  'Sites',
  {
    ...
    saving: (self) => {
      ...
      if (self.isNew()) {
        ...
        if (remult.user) self.createUser = remult.user.id
      } else {
        if (self.$.id.valueChanged()) self.$.id.error = 'not allowed to change'
      }
    }
  }
...
export class SitesEntity extends EntityBase {
  ...
  static async completeInit() {
    ...
    let missingInDb = Sites.schemas.filter(
      (siteFromEnv) => !sites.find((y) => y.id == siteFromEnv)
    )
    ...
    for (const s of sites) {
      if (!Sites.schemas.includes(s.id)) Sites.schemas.push(s.id)
    }
  }
}
