...
export class SendBulkSms {
  ...
  sendSingleHelperButton(ui: UITools) {
    return {
            ...
            if (h.doNotSendSms) {
              if (
                !(await ui.YesNoPromise(
                  'המתנדב ביקש לא לקבל הודעות, האם לשלוח בכל זאת?'
                ))
              )
                return
            }
            if (
              (await remult.repo(HelperCommunicationHistory).count({
                volunteer: h,
              ...
              if (
                !(await ui.YesNoPromise(
                  'כבר נשלחה למתנדב הודעה מאתמול בבוקר, האם לשלוח בכל זאת?'
                ))
              )
                return
            }
      ...
      visible: () =>
        remult.isAllowed(Roles.admin) && getSettings().bulkSmsEnabled
    }
  }
  ...
  private async editInviteMethod(
    ...
    await ui.editCustomMessageDialog({
      ...
      helpText: '',
      title:
        settings.lang.sendMessageToInviteVolunteers +
        ' ' +
        settings.lang.to +
        ' ' +
        args.messageCount +
        ' ' +
        settings.lang.volunteers,
      buttons: [
        {
          name: 'שלח הודעה',
  ...
  private async sendSingleMessage(helperId: string) {
    ...
    if (!settings.bulkSmsEnabled) throw 'Forbidden'
    let message = this.buildMessage(helper.name, settings).merge(
      settings.inviteVolunteersMessage
    )
    if (true)
      await new SendSmsUtils().sendSms(helper.phone.thePhone, message, helper)
  ...
  async getVolunteers() {
        ...
        where: async () => [
          await Filter.translateCustomWhere(
            helpers.where({
              ...
              isFrozen: false,
              $and: [
                this.city && Helpers.deliveredPreviously({ city: this.city })
              ]
            }),
            helpers.metadata,
            remult
    ...
    console.log(q)
    return (await db.execute(q)).rows as {
      phone: string
      name: string
      id: string
    }[]
