  ...
  static async getAllEvents(
    ...
    for (const org of schemas) {
      ...
      if (s) {
        ...
        query += await sql.build(
          'select ',
          ["'" + org + "' site"],
          ' from ',
          org + '.' + (await e.metadata.getDbName()),
          ' where ',
          [
            e.where({
    ...
    if (query) {
      let sites = (
        await getDb().execute(' select distinct site from (' + query + ') x')
      ).rows.map((x) => x.site)
      ...
      for (const org of sites) {
        ...
        if (
          !settings.donotShowEventsInGeneralList &&
          !settings.forWho.args.leftToRight
        ) {
