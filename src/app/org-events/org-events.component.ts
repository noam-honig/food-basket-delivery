...
export class OrgEventsComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit() {
    ...
    this.unObserve =
      await RegisterToEvent.volunteerInfoChanged.dispatcher.observe(
        async () => {
          if (this.isGuest) {
            ...
            this.events = await OrgEventsController.getEvents(
              RegisterToEvent.volunteerInfo.phone,
      ...
      xlsx.utils.json_to_sheet(
        this.events.map((e) => {
          const url =
            ...
            `/guest/event/${e.site}/${e.id}/${e.remoteUrl ? 'remote' : ''}`
          return {
            'שם התנדבות': e.name,
            'סוג התנדבות': 'פומבי',
