...
export class MapComponent implements OnInit, OnDestroy {
  ...
  async loadPotentialAsigment(
    ...
    let families = await DistributionMapController.GetDeliveriesLocation(
      true,
      city,
      group,
      distCenter,
    ...
    try {
      ...
      if (!this.hasFamilies && this.helper) {
        if (this.helper.preferredDistributionAreaAddressHelper.ok)
          this.map.setCenter(
            this.helper.preferredDistributionAreaAddressHelper.location
          )
  ...
  setFamilyOnMap(familyId: string, lat: number, lng: number) {
    ...
    if (marker && marker.getMap() == null) marker = undefined
    if (!marker) {
      ...
      google.maps.event.addListener(marker, 'click', async () => {
        ...
        if (families.length > 0) this.userClickedOnFamilyOnMap(families)
  ...
  fitBounds() {
    if (this.disableMapBoundsRefrest) return
    if (!this.map) return
  ...
  async test(families: ActiveFamilyDeliveries[], h: HelpersBase) {
    ...
    if (this.helper != helper) {
      ...
      this.helperMarkers.push(
        new google.maps.Marker({
          map: this.map,
          position: start,
          icon: 'https://labs.google.com/ridefinder/images/mm_20_purple.png'
        })
      )
      this.helper = helper
      if (helper.preferredDistributionAreaAddressHelper.ok) {
        this.helperMarkers.push(
          new google.maps.Marker({
            map: this.map,
            position: helper.preferredDistributionAreaAddressHelper.location,
      ...
      if (helper.preferredFinishAddressHelper.ok) {
        this.helperMarkers.push(
          new google.maps.Marker({
            map: this.map,
            position: helper.preferredFinishAddressHelper.location,
            icon: 'https://maps.google.com/mapfiles/arrow.png'
          })
        )
      }
    ...
    families.forEach((f) => {
      ...
      if (pi >= 0) prevFamilies.splice(pi, 1)
      let marker = this.setFamilyOnMap(
        f.id,
        f.addressLatitude,
        f.addressLongitude
      )
      try {
        if (f.deliverStatus == DeliveryStatus.ReadyForDelivery)
          this.bounds.extend(marker.getPosition())
        else secondaryBounds.extend(marker.getPosition())
      } catch (err) {
      ...
      switch (f.deliverStatus) {
        case DeliveryStatus.ReadyForDelivery:
          ...
          if (
            ...
            JSON.stringify(prevMarker.getPosition()) !=
        ...
        case DeliveryStatus.FailedOther:
          marker.setIcon(
            'https://maps.google.com/mapfiles/ms/micons/red-pushpin.png'
          )
          break
      }
    })
    ...
    if (this.bounds.isEmpty()) this.bounds = secondaryBounds
