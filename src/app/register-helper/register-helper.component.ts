...
export class RegisterHelperComponent implements OnInit {
  ...
  area = new DataAreaSettings({
    fields: () => [
      {
        field: this.helper.$.socialSecurityNumber,
        caption: 'תעודת זהות (עבור ביטוח מתנדבים)'
      },
  ...
  async submit() {
    ...
    try {
      ...
      gtag('event', 'conversion', {
        send_to: 'AW-452581833/e8KfCKWQse8BEMmz59cB'
      })
    ...
    await openDialog(
      ...
      (x) => (x.args = { question: 'תודה על עזרתך', showOnlyConfirm: true })
    )

    if (this.refer) return
    if (this.helper.referredBy != '')
      window.location.href = this.helper.referredBy
