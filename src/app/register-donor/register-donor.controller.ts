...
export class donorForm {
  ...
  @Field<donorForm, Phone>({
    ...
    inputType: 'tel',
    validate: (self, col) => {
      if (!col.value || col.value.thePhone == '') col.error = 'אנא הזן ערך'
  ...
  async createDonor() {
    ...
    async function addDelivery(
      ...
      if (q > 0) {
        ...
        await Families.addDelivery(
          f.id,
          await remult.repo(BasketType).findId(type),
          null,
          null,
          {
            comment: '',
    ...
    else await addDelivery('מחשב_ישן', this.computer, this.selfDeliver)

    if (this.laptopAge === undefined || this.laptopAge.isNew)
      await addDelivery('לפטופ', this.laptop, this.selfDeliver)
    else await addDelivery('לפטופ_ישן', this.laptop, this.selfDeliver)
    ...
    if (quantity == 0) {
      await Families.addDelivery(
        f.id,
        await remult.repo(BasketType).findId('לא פורט'),
        null,
        null,
        {
          comment: '',
    ...
    if (
      settings.registerFamilyReplyEmailText &&
      settings.registerFamilyReplyEmailText != ''
    ) {
      let message = SendSmsAction.getMessage(
        settings.registerFamilyReplyEmailText,
