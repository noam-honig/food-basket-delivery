...
export class RegisterDonorComponent implements OnInit {
  ...
  showCCMessage(): boolean {
    if (
      this.activeRoute.routeConfig.data &&
      this.activeRoute.routeConfig.data.isCC
    )
      return true
    else return false
  }
  ...
  area = new DataAreaSettings({
    fields: () => [
      ...
      this.donor.$.screen,
      this.donor.$.donationType,
      { field: this.donor.$.phone, click: null },
  ...
  hasMandatoryFields() {
    return (
      ...
      this.donor.phone &&
      isPhoneValidForIsrael(this.donor.phone.thePhone) &&
  ...
  hasEnough() {
    let total =
      (this.donor.laptop != undefined ? this.donor.laptop : 0) +
  ...
  async submit() {
    ...
    if (!this.hasEnough()) {
      this.dialog.Error(
        'לצערינו לא נוכל לאסוף תרומות עם פחות מ-' +
          RegisterDonorComponent.MinQuantity +
          ' פריטים. נשמח אם תביאו את הציוד אל אחת מנקודות האיסוף שלנו'
      )
      return
    }
    try {
      ...
      try {
        ...
        gtag('event', 'conversion', {
          send_to: 'AW-452581833/GgaBCLbpje8BEMmz59cB'
        })
