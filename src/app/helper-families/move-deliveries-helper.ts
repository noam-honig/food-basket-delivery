...
export class moveDeliveriesHelper {
  ...
  async move(
    from: HelpersBase,
    to: HelpersBase,
    showToHelperAssignmentWhenDone: boolean,
    extraMessage = '',
    allowSelect = false
  ) {
    ...
    if (deliveries.length > 0) {
      ...
      if (!allowSelect) {
        if (
          await this.dialog.YesNoPromise(
            extraMessage +
              ' ' +
              this.settings.lang.transfer +
              ' ' +
              deliveries.length +
              ' ' +
              this.settings.lang.deliveriesFrom +
              '"' +
              from.name +
              '"' +
              ' ' +
              this.settings.lang.toVolunteer +
              ' ' +
              '"' +
              to.name +
              '"'
      ...
      if (selectedDeliveries.length > 0) {
        let message =
          await moveDeliveriesHelperController.moveDeliveriesBetweenVolunteers(
            selectedDeliveries.map((x) => x.id),
            to
          )
        if (message) {
