...
export class CallerComponent implements OnInit {
    ...
    },
    {
      ...
      click: () => {
        this.dialog.inputAreaDialog({
          fields: [
            this.settings.$.callModuleMessageText,
            this.settings.$.callModuleMessageLink
          ],
          ok: () => {
            this.settings.save()
          },
  ...
  ngOnInit(): void {
    ...
    this.dialog.donotWait(
      async () =>
        (this.caller = await remult.repo(Callers).findId(remult.user.id))
    )
  }
  caller: Callers
  async nextCall() {
    if (!(await this.controller.nextCall())) {
      ...
      this.d = undefined
    } else this.dialog.Info('עובר למשפחה הבאה...')
  ...
  async interested() {
    if (
      await this.dialog.YesNoPromise('האם לעדכן כמעוניינים ולעבור למשפחה הבאה?')
    ) {
  ...
  async loadCall() {
    this.d = await remult
      ...
      .findFirst(ActiveFamilyDeliveries.inProgressCallerDeliveries())
    if (this.caller)
      this.dialog.donotWait(
        async () => (this.caller = await this.caller._.reload())
      )
  }
  ...
  async updateFamilyInfo() {
    ...
    this.dialog.inputAreaDialog({
      ...
      fields: [
        ...
        f.address,
        [f.appartment, f.floor],
        [f.entrance, f.buildingCode],
        f.addressComment,
