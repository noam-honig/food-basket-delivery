...
export class CallerController {
  ...
  async selectFamily(f: CallerFamilyInfo) {
    ...
    const fd = await remult.repo(ActiveFamilyDeliveries).findId(f.deliveryId)
    if (fd && fd.deliverStatus == DeliveryStatus.enquireDetails) {
      fd.caller = await remult.context.getCurrentUser()
  ...
  async releaseCurrentFamily() {
    const repo = remult.repo(ActiveFamilyDeliveries)
    for (const fd of await repo.find({
      where: FamilyDeliveries.inProgressCallerDeliveries()
    })) {
      fd.caller = null
      await fd.save()
    }
  ...
  async nextCall() {
    ...
    const helper = await remult.context.getCurrentUser()
    async function findDelivery(where: EntityFilter<ActiveFamilyDeliveries>) {
      for await (const fd of repo.query({
        where,
        orderBy: { lastCallDate: 'asc' }
      })) {
        if (groupMatches(helper, fd)) return fd
      }
    }
    let fd = await findDelivery({ $and: [where, { lastCallDate: null }] })
    if (!fd) {
    ...
    return (
      await remult.repo(ActiveFamilyDeliveries).find({
        where: {
          deliverStatus: DeliveryStatus.enquireDetails,
          $or: [
            ActiveFamilyDeliveries.filterPhone(search),
            {
      ...
      .filter((x) => groupMatches(helper, x))
      .filter((x, i) => i < 30)
      .map((fd) => ({
        deliveryId: fd.id,
        name: fd.name,
        phone: [fd.phone1, fd.phone2, fd.phone3, fd.phone4]
          .filter((x) => x)
