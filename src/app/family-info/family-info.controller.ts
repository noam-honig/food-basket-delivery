...
export class FamilyInfoController {
  static createPhoneProxyOnServer: (
    phone1: string,
    phone2: string
  ...
  static async privateCall(deliveryId: string): Promise<{
    ...
    try {
      ...
      if (!settings.usePhoneProxy) throw 'פרוקסי לא מופעל לסביבה זו'
      let fd = await remult.repo(ActiveFamilyDeliveries).findId(deliveryId)
      if (!fd) throw 'משלוח לא נמצא'
      if (
        !fd.courier.isCurrentUser() &&
        !remult.isAllowed([Roles.admin, Roles.distCenterAdmin])
      )
      ...
      if (!h) throw 'מתנדב לא נמצא'
      let vPhone = h.phone.thePhone
      if (vPhone.startsWith('0')) vPhone = vPhone.substring(1)
      vPhone = '+972' + vPhone

      let r = await FamilyInfoController.createPhoneProxyOnServer(
        cleanPhone,
        vPhone
      )
  ...
  static async ShowFamilyTz(deliveryId: string) {
    ...
    if (!s.showTzToVolunteer) return ''
    var d = await remult.repo(ActiveFamilyDeliveries).findId(deliveryId)
    if (!d) return
    if (
      !d.courier.isCurrentUser() &&
      !remult.isAllowed([Roles.admin, Roles.distCenterAdmin])
    )
      return ''

    var f = await Families.getSpecificFamilyWithoutUserRestrictionsBackendOnly(
      d.family
    )
    if (!f) return ''
    return f.name + ':' + f.tz
